...
Marlin does support older versions of Perl, but this manual is written with
modern Perl in mind.
...
version) at the top of your file. If you are targeting older versions
...
Here C<< $user >> is an object and C<< change_password_to >> is a method that
...
(The C<< qw() >> operator is a simple way to create a list of words
...
Adding the exclamation mark indicates that an attribute is required.
...
a class, we just define subs in the class. Each method takes the object
...
people to update it. Accessor methods are sometimes also called I<mutators>.
...
When calling the accessor method without any extra arguments, it acts like
...
(like in Example 8) are I<< lazy defaults >>. This means that the
constructor ignores the default, and instead the reader
